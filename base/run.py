...
class Run:
    ...
    def __init__(self, platform=Conf().android_name):
    ...
    def exec(self, sys_argv):
        ...
        if len(sys_argv) != 0:
            ...
            dir_tests = os.path.basename(os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, self.env.path.get('tests'))))
    ...
    def _scheduling_process(self):
        ...
        tests_path = str(os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, self.env.path.get('tests'))))

        suit_name_list = ls_by_key(tests_path, 'test_')

        if len(suit_name_list) == 0 or len(self.devices) == 0:
            raise IndexError('data is bad')

        task_num, mod = divmod(len(suit_name_list), len(self.devices))

        suitname_list_slice = []
        tmp = []
        count = 0
        for suitname in suit_name_list:
            tmp.append(suitname)
            if count < (len(suit_name_list) - mod):
                if len(tmp) == task_num:
