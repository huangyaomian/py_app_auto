...
class ArgsData():
    def __init__(self):
        self.config_path = str(
            os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, "data/args_data.yaml")))
        self.info = self.get_info()
...
def ls_by_key(path: str, key: str):
    ...
    # result = ['test_suit2', 'test_usercenter', 'test_vedio']
    result = ['test_suit2']

    # out = Shell.invoke('ls', cwd=path, is_log=True)
    out = Shell.invoke('dir', cwd=path, is_log=False, encoding='gbk')
